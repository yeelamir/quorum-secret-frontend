<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-100">
    <div class="w-full max-w-xl p-6 bg-white rounded-lg shadow-lg relative">
      ...
      <div class="mt-6 text-sm text-blue-600 cursor-pointer" @click="showMembers = !showMembers">
        {{ showMembers ? 'Hide members' : 'Show members' }}
      </div>

      <div v-if="showMembers" class="mt-4 max-h-36 overflow-y-auto border border-gray-300 rounded">
        <table class="w-full text-sm text-left text-gray-700">
          <thead class="bg-gray-100 text-xs uppercase text-gray-600">
            <tr>
              <th scope="col" class="px-4 py-2">User Name</th>
              <th scope="col" class="px-4 py-2">Owner</th>
              <th scope="col" class="px-4 py-2">Requested</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="member in members" :key="member.UserId" class="border-t">
              <td class="px-4 py-2">{{ member.UserName }}</td>
              <td class="px-4 py-2">{{ member.IsOwner ? 'Yes' : 'No' }}</td>
              <td class="px-4 py-2">{{ member.DecryptRequest ? '✅' : '❌' }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
onMounted(async () => {
  ...
  const res = await axios.get(API_HOST + `secrets/members/${route.params.secretId}`, {
    headers: { Authorization: `Bearer ${token}` }
  });
